...
// ───────────────────────────────────────────────────────────────────────────
const SITE_ID = "abogados-familia";
...
// Colores (ajustá a la marca del sitio):
const BG = "#1e3a5f";
const FG = "#ffffff";
const BTN_BG = "#c9a227";
const BTN_FG = "#1e2a16";
        ...
        {status === "success" ? (
          ...
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 12, justifyContent: "center" }}>
            <p style={{ fontSize: 14, margin: 0, lineHeight: 1.35, flex: "1 1 260px" }}>
              👨‍👩‍👧 <strong>No te quedes sin saber tus derechos de familia.</strong> Pensión, divorcio, cuidado personal — guías claras a tu correo, sin spam.
            </p>
            <form onSubmit={submit} style={{ display: "flex", gap: 8, flex: "0 0 auto" }}>
              ...
              <button type="submit" disabled={status === "loading"}
                ...
                {status === "loading" ? "..." : "Quiero recibirlas"}
              </button>
            </form>
            {status === "error" && (
              <span style={{ fontSize: 12, opacity: 0.9 }}>Error, intentá de nuevo.</span>
            )}
          </div>
        )}
